...
const selectedCinema = cinemas.find((result) => result.id === Number(selectedScreeningRaw[1]));
const movies = getData("movieData");
const selectedMovie = movies.find((result) => result.uuid === Number(selectedScreeningRaw[0]));
const selectedScreening = selectedCinema.movieList.find((result) => result.uuid === Number(selectedScreeningRaw[3]));
...
const renderScreeningInNavbar = (selectedMovie, selectedCinema, selectedScreening) => {
    const screeningDate = new Date(selectedScreening.screenDate);
    const dateString = screeningDate.toLocaleString("default", {
        weekday: "long",
        day: "2-digit",
        month: "short",
    });
...
function updateSeatTakenStatus(seats, selectionIndex) {
    seats.forEach((seat) => {
        const foundIndex = selectionIndex.findIndex((data) => seat.line === data.line && seat.seatID === data.seatID);
...
function seatsRender(seats) {
    ...
    for (let line = 1; line <= 6; line++) {
        ...
        for (let seat = 1; seat <= seatsPerLine; seat++) {
            ...
            const isSelected = selectedSeats.some((s) => s.line === line && s.seat === seat);
...
const enableSeatsSelection = () => {
    const allSeats = document.querySelectorAll(".venue__seat");
    allSeats.forEach((seat) => {
        seat.addEventListener("click", () => {
            const selectedSeat = seat.classList[1].split("-");
            const line = Number(selectedSeat[0]);
            const seatID = Number(selectedSeat[1]);
            const seatIndex = selectedSeats.findIndex((rs) => rs.line === line && rs.seat === seatID);
...
const displayMovieTicket = () => {
    const selectedLines = selectedSeats.reduce((lines, seat) => {
 ...
 <div class="ticket">
    ...
    const formInputs = paymentForm.querySelectorAll("input:not([type='submit'])");
